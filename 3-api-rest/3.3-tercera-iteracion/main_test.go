...
func noSeHaProducidoNingunError() error {
	if e != nil {
		return errors.New(fmt.Sprintf("unexpected error: %s", e.Error()))
	}

	return nil
}

func usuarioBorraElLibroConId(id string) (err error) {
...
func elLibroConIdYTituloHaSidoBorrado(id string) (err error) {
	req, err := http.NewRequest(http.MethodGet, "/books/" + id, nil)
	if err != nil {
		return
	}
